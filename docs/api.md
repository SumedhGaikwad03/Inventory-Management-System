...
|---|---|---|---|---|
| `name` | string | Yes | Product display name | Max 100 characters (`nvarchar(100)`) |
| `quantity` | integer | Yes | Initial stock quantity | >= 0 |
| `price` | number | Yes | Unit price | >= 0.00, precision `decimal(18,2)` |
| `categoryId` | integer | Yes | Foreign key of assigned category | Must reference an existing category |
...
- `400 Bad Request` - Validation error (e.g. name exceeds 100 characters) or referenced category ID does not exist.
- `401 Unauthorized` - Missing or invalid token.
...
- `400 Bad Request` - Validation failure (e.g. name exceeds 100 characters) or referenced category does not exist.
- `401 Unauthorized` - Missing or invalid token.
- `403 Forbidden` - User is not an Admin.
...
| Field | Type | Required | Description | Constraints |
|---|---|---|---|---|
| `name` | string | Yes | Category display name | Max 100 characters (`nvarchar(100)`), unique |
| `description` | string | No | Optional category summary | Max 1000 characters (`nvarchar(1000)`) |
...
- `400 Bad Request` - Category name already exists, exceeds 100 characters, description exceeds 1000 characters, or required `name` field is missing.
- `401 Unauthorized` - Missing or invalid token.
- `403 Forbidden` - User is not an Admin.
...
- `400 Bad Request` - Category name conflict with another existing category or validation error (e.g. name exceeds 100 characters, description exceeds 1000 characters).
- `401 Unauthorized` - Missing or invalid token.
- `403 Forbidden` - User is not an Admin.
...
| Field | Type | Required | Description | Constraints |
|---|---|---|---|---|
| `productId` | integer | Yes | ID of the target product to adjust | Must reference an existing product |
| `quantityChanged` | integer | Yes | Units to adjust (positive or negative) | Non-zero integer; must not result in negative stock |
| `transactionType` | string | Yes | Operation category tag | Max 20 characters (`nvarchar(20)`). Supported: `Restock`, `Return`, `Sale`, `Damage`, `Adjustment` |
...
| `Adjustment` | Positive or Negative | Adjusts stock | Manual stock level correction or audit reconcile |
...
- **Name Length Limits**: Both `Category.Name` and `Product.Name` are limited to a maximum of 100 characters in request DTO validation and the database schema (`nvarchar(100)`).
- **Category Name Uniqueness**: Category names must be unique across the catalog. Duplicate names are rejected by application validation with `400 Bad Request` and guaranteed at the database level by a unique index (`IX_Categories_Name`).
